...
RSpec.describe PPU do
  ...
  describe '#step' do
    ...
    context 'LCD が OFF (LCDC bit7=0) のとき' do
      ...
      before { mmu.write_u8(address: 0xFF40, value: 0x00) } # LCDC bit7=0
      let(:cycles) { 1000 }

      it 'LY は 0 のまま、LY レジスタも 0' do
        subject
        expect(ppu.ly).to eq 0
        expect(mmu.read_u8(address: 0xFF44)).to eq 0
      end
    end

    context 'LCD が ON (LCDC bit7=1) のとき' do
      before { mmu.write_u8(address: 0xFF40, value: 0x80) } # LCDC bit7=1
      ...
      context '456 サイクル進めたとき(ちょうど 1 ライン)' do
        ...
        it 'LY=1 になり、MMU の 0xFF44 にも反映される' do
          ...
          expect(mmu.read_u8(address: 0xFF44)).to eq 1
        end
      end
      ...
      context '456 * 144 サイクル進めたとき(VBlank 開始ライン)' do
        ...
        it 'LY=144 になる(VBlank 開始)' do
          ...
          expect(mmu.read_u8(address: 0xFF44)).to eq 144
        end
      end
      ...
      context '小さなサイクルを複数回呼んだとき' do
        ...
        before do
          mmu.write_u8(address: 0xFF40, value: 0x80)
  ...
  describe '#step(456) による BG タイル描画(C-2)' do
    ...
    before do
      mmu.write_u8(address: 0xFF40, value: 0x91)
    end

    context 'BG が無効(LCDC bit0=0)のとき' do
      before do
        mmu.write_u8(address: 0xFF40, value: 0x90) # LCD ON だが BG OFF
    ...
    context 'タイル 0 を全色 3(黒)で塗りつぶしたとき' do
      ...
      before do
        16.times { |i| mmu.write_u8(address: 0x8000 + i, value: 0xFF) } # タイル 0 全画素を色 3
        # タイルマップの 1 行目を全部タイル 0 にする(20 タイル分で画面横が埋まる)
        20.times { |col| mmu.write_u8(address: 0x9800 + col, value: 0x00) }
        # BGP: 色番号 3 → 0b11(明度 3、黒)
        mmu.write_u8(address: 0xFF47, value: 0xFC) # 0b11_11_11_00 = 色 0→白、色 1〜3 → 黒
      end
    ...
    context 'タイル 0 を全色 0(白)で塗りつぶしたとき' do
      ...
      before do
        ...
        mmu.write_u8(address: 0xFF47, value: 0xFC) # 色 0 → 白
      end
    ...
    context 'パレット(BGP)を反転したとき' do
      ...
      before do
        16.times { |i| mmu.write_u8(address: 0x8000 + i, value: 0xFF) } # 全画素 color_id=3
        20.times { |col| mmu.write_u8(address: 0x9800 + col, value: 0x00) }
        # BGP: 色番号 3 → 0b00(白)、色番号 0 → 0b11(黒)に反転
        mmu.write_u8(address: 0xFF47, value: 0b00_01_10_11)
      end
    ...
    context '左端ピクセルが色 1、右端ピクセルが色 2 のタイルのとき' do
      ...
      before do
        mmu.write_u8(address: 0x8000, value: 0x80)     # 1 行目 byte_lo
        mmu.write_u8(address: 0x8001, value: 0x01)     # 1 行目 byte_hi
        mmu.write_u8(address: 0x9800, value: 0x00)     # マップ位置 (0,0) にタイル 0
        # BGP: 恒等パレット(色番号 = 明度)
        mmu.write_u8(address: 0xFF47, value: 0b11_10_01_00)
      end
    ...
    context 'LCDC bit3=1(タイルマップ 0x9C00)を指定したとき' do
      ...
      before do
        mmu.write_u8(address: 0xFF40, value: 0x99) # 0b1001_1001(bit3=1)
        16.times { |i| mmu.write_u8(address: 0x8000 + i, value: 0xFF) }
        # 0x9800 側にはタイル 0 を置かない(マップが切り替わったか確認用)
        20.times { |col| mmu.write_u8(address: 0x9C00 + col, value: 0x00) }
        mmu.write_u8(address: 0xFF47, value: 0xFC)
      end
    ...
    context 'LCDC bit4=0(signed アドレッシング)で tile_num=0xFF を参照したとき' do
      ...
      before do
        mmu.write_u8(address: 0xFF40, value: 0x81) # 0b1000_0001(bit4=0, bit0=BG ON, bit7=LCD ON)
